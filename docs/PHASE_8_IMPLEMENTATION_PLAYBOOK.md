...
| D4 | **Secret scanning tool** — C2 mentions both `detect-secrets` and `gitleaks` as options for the pre-commit hook. They have different false-positive profiles and maintenance cadences. | Before Track C2 (configuration audit) | `gitleaks` — native git hook integration, active maintenance, good Python project defaults, and TOML-based allowlist configuration. Run `detect-secrets` as a secondary CI-only check if desired. | `detect-secrets` requires a baseline file that can drift. `gitleaks` scans against rules that are easier to manage and version. Either tool catches the same classes of secrets; `gitleaks` is lower-maintenance as a pre-commit hook. The allowlist (`.gitleaks.toml`) should be reviewed as part of the security review. |
| D5 | **Performance benchmark workload definition** — E3 says "run a performance benchmark" but does not define the workload. Without a defined workload, the baseline is not reproducible or comparable across hardware. | Before Track E3 (performance baseline) | Standard workload: 10 concurrent cases, mixed seed types (4 IOC, 3 alert, 2 anomaly, 1 hunt query), default branch budget (max-depth 3, max-branches 10), all Tier-1 adapters enabled, Tier-2 adapters disabled. Run on the reference hardware profile from `PROJECT_PLAN.md` (≥1× H100/H200 or 2× L40S). | The workload must be representative enough to expose bottlenecks but not so large that it takes hours to run. 10 concurrent cases at default budget is achievable in under 30 minutes on reference hardware and exercises the concurrency model meaningfully. |
